...
def main():
    ...
    parser.add_argument("--volume-tol", type=float, default=0.5, help="Volume tolerance, %% (default 0.5%%)")
    parser.add_argument("--com-tol", type=float, default=0.1, help="COM tolerance, mm (default 0.1 mm)")
    ...
    print(f"{tr(lang, 'cli_reading')} {args.file_a} ...")
    tree_a = parse_step(args.file_a)
    print(f"{tr(lang, 'cli_reading')} {args.file_b} ...")
    tree_b = parse_step(args.file_b)

    print(f"{tr(lang, 'cli_comparing')} ...")
    ...
    print(f"{tr(lang, 'cli_saved')} {args.output}")
